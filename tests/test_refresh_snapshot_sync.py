...
def test_healthcheck_resets_consecutive_failure_count_after_success():
    ...
    assert 'MAX_BOARD_AGE_MINUTES="${REDFOX_MAX_BOARD_AGE_MINUTES:-10}"' in script
    assert "/refresh anomaly board DONE/ { failures=0; next }" in script
    assert "/refresh anomaly board ERROR/ { failures++ }" in script
    assert 'board_source_age=$(json_age_minutes "$DATA/freshness.json" board_oldest_ts)' in script
    assert 'issues+=("board source freshness ${board_source_age}m")' in script


def test_customer_freshness_threshold_enforces_the_ten_minute_target():
    board = (Path(__file__).resolve().parents[1] / "site" / "board.html").read_text(encoding="utf-8")
    assert "label:'Board source', ts:f.board_oldest_ts||f.dk_ts, fresh:10, warn:12" in board
    assert "label:'Board publish', ts:f.board_published_at||f.engine_ts, fresh:10, warn:12" in board
