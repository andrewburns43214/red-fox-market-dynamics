...
# Keep latest per sport/game/side
snap = (
    snap.sort_values("timestamp")
        .groupby(["sport","game_id","side"])
        .tail(1)
)

# Infer market type from side string
def infer_market(side):
    if isinstance(side, str):
        if re.search(r'Over|Under', side):
            return "TOTAL"
        elif re.search(r'[+-]\d+\.\d+', side):
            return "SPREAD"
        else:
            return "MONEYLINE"
    return None

snap["market_display"] = snap["side"].apply(infer_market)
...
print("\n==============================================")
print("TONIGHT — CONFIDENCE vs CURRENT ODDS (FIXED)")
print("==============================================")
