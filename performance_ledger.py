...
def _frozen_records(frozen: pd.DataFrame, history: pd.DataFrame, source_name: str) -> list[dict]:
    ...
    if frozen.empty:
        return records
    ordered = frozen.copy()
    ordered["_frozen"] = pd.to_datetime(ordered.get("frozen_at_utc", ""), errors="coerce", utc=True)
    ordered["_state_at"] = pd.to_datetime(ordered.get("final_pregame_state_at_utc", ""), errors="coerce", utc=True)
    ordered["_kickoff"] = pd.to_datetime(ordered.get("kickoff_iso", ""), errors="coerce", utc=True)
