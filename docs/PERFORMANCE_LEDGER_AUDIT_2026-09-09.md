...
### Timer diagnosis and third controlled deployment

The stale state was `/var/lib/systemd/timers/stamp-redfox-performance.timer`, with an mtime of September 9 at 11:11:15 PM ET. The former unit combined `OnBootSec=2min`, `OnUnitActiveSec=2min`, and `Persistent=true`. Because the server had been running since January, the boot-relative first deadline was already elapsed; the persistent stamp then restored the failed first deployment's trigger instead of creating a fresh activation-relative deadline.

The timer-only correction uses `OnActiveSec=15s`, `OnUnitActiveSec=2min`, and no persistence. Accelerated temporary systemd tests proved that it populated NEXT, ran again after a deliberately failed first service, advanced NEXT, and scheduled a fresh first execution after stop/restart.

In the third production attempt, the identified stale stamp was removed before timer activation. Production evidence:

- timer started `active (waiting)` with NEXT at 11:55:29 PM ET;
- first scheduled invocation ran 11:55:31–11:55:33 PM ET;
- second scheduled invocation ran 11:57:34–11:57:35 PM ET;
- NEXT advanced to 11:59:34 PM ET;
- both runs completed successfully and preserved 9 Supported Side rows and 4 Favorite v2 rows.

The next gate failed when the public unauthenticated Supported Side CSV request returned HTTP 200 after the admin Nginx configuration was loaded, even though the verifier's direct `/verify-admin` request without a token returned HTTP 401. The rollout stopped immediately. No admin-positive, live-kickoff, or postgame production checks followed. The third deployment was fully rolled back by commit `cb2dea1`; the timer/service, timer stamp, seed files, admin routes, and verifier changes were removed, and Nginx and the customer site were confirmed healthy.
